...
class FlxSlider extends #if (flixel < "5.7.0") FlxSpriteGroup #else FlxSpriteContainer #end
{
	...
	{
		var pos:Float = y + offset.y + ((_height - handle.height) * ((value - minValue) / (maxValue - minValue)));

		// Make sure the pos stays within the bounds
		if (pos > y + _height + offset.y)
		{
			pos = y + _height + offset.y;
		}
	...
	{
		var pos:Float = (handle.y - y - offset.y) / (_height - handle.height);
	...
	{
		if (_bounds != null)
			_bounds.set(x + offset.x, y + offset.y, _width + 30, _height);
	}
}
#end
